...
option_list = list(
  ...
  make_option(
   ...
   make_option(
    ...
    default = 'FALSE',
    type = 'character',
    help = "Is the input batch and data centered"
	 ),
   make_option(
    c("-y", "--scaleY"),
    action = "store",
...
if (opt$filt == "") {filt<-NULL} else {filt<-opt$filt};

if (opt$seed == "") {seed<-NULL} else {seed<-opt$seed};

out<-gPCA.batchdetect(x=data, batch=batch, center=center, scaleY=scaleY, nperm=nperm, filt=filt, seed=seed)
...
png(paste(opt$path,'/guided_comp_3.png', sep=""), width=1020, height=800, units='px')
PCplot(out,ug="guided",type="comp",npcs=3)
invisible(dev.off())

par(mai=c(0.65,0.65,0.1,0.1),cex=0.8)
png(paste(opt$path,'/unguided_comp_3.png', sep=""), width=1020, height=800, units='px')
...
invisible(dev.off())
     ...
     write(paste('<html>
      ...
      <center><title>Guided Compare to 3</title><br><img src="guided_comp_3.png"/></center><br>
      <center><title>Unguided Compare to 3</title><br><img src="unguided_comp_3.png"/></center><br>
